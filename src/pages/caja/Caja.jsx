  ...
  async function guardarMov(e) {
    ...
    try {
      ...
      if (!docC) {
        ...
        await setDoc(ref, data); setDocC(data)
      ...
      setFM({ tipo:'entrada-efectivo', desc:'', importe:'', kineId:'' }); setModal(false)
      ...
      {!cargado ? (
        <div className="card">
          <div className="empty-search">
            <ILupa/><p>Elegí el mes y hacé clic en "Ver caja"</p>
              ...
              <div className="card" style={{padding:0,overflow:'hidden'}}>
                <div className="tw">
                  <table>
                    ...
                    <tbody>
                      {!filtroDia && saldoI > 0 && (
                        <tr><td className="cgr">—</td><td><span className="badge bb">Saldo inicial</span></td><td>—</td><td>—</td><td className="cve fw6">{fmtMonto(saldoI)}</td><td className="fw6">{fmtMonto(saldoI)}</td><td className="cgr">Sistema</td></tr>
                      )}
                      {movsVis.length === 0 && <tr><td colSpan="7" className="emt">{filtroDia?'No hay movimientos para este día':'Sin movimientos este mes'}</td></tr>}
                      {movsVis.map((m, i) => {
