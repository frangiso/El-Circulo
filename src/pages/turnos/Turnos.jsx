    ...
    return (
      <div>
        <div className="row" style={{ marginBottom: 12, justifyContent: 'space-between' }}>
          <button className="btn bs bsm" onClick={semanaAnterior}>← Anterior</button>
          <span style={{ fontSize: 13, fontWeight: 600 }}>{fmtFechaCor(semana[0])} — {fmtFechaCor(semana[6])}</span>
          <button className="btn bs bsm" onClick={semanaSiguiente}>Siguiente →</button>
        </div>
        {cargando ? (
          <div className="sc"><div className="sp" /></div>
        ) : horasSet.length === 0 ? (
          <div className="card"><div className="emt">No hay turnos esta semana</div></div>
        ) : (
                ...
                {horasSet.map(hora => (
                  <tr key={hora}>
                    ...
                    {semana.map(f => {
                      ...
                      return (
                        <td key={f} style={{ padding: '4px 5px', border: '1px solid #eee', verticalAlign: 'top', background: f === hoy2 ? '#fafeff' : 'white' }}>
                          {celdaTurnos.map(t => {
                            const pac = mapaP[t.pacienteId]
                            const estP = pac?.plan ? estadoPlan(pac.plan) : null
                            const bg = t.asistencia === 'asistio' ? '#f0fff4' : t.asistencia === 'falto' ? '#fff5f5' : estP === 'vencido' ? '#fff8f8' : 'white'
                            return (
    ...
    <div>
      ...
      <div className="row" style={{ marginBottom: 14, justifyContent: 'space-between', flexWrap: 'wrap', gap: 8 }}>
        <div style={{ display: 'flex', border: '1px solid #ddd', borderRadius: 8, overflow: 'hidden' }}>
          <button onClick={() => cambiarVista('dia')} style={{ padding: '7px 18px', fontSize: 13, fontWeight: 600, cursor: 'pointer', border: 'none', background: vista === 'dia' ? 'var(--az)' : '#fff', color: vista === 'dia' ? '#fff' : '#666' }}>
            Vista día
          </button>
          <button onClick={() => cambiarVista('semana')} style={{ padding: '7px 18px', fontSize: 13, fontWeight: 600, cursor: 'pointer', border: 'none', borderLeft: '1px solid #ddd', background: vista === 'semana' ? 'var(--az)' : '#fff', color: vista === 'semana' ? '#fff' : '#666' }}>
            Vista semana
          </button>
        </div>
        {vista === 'semana' && (
          <select value={kineFiltro} onChange={e => setKF(e.target.value)} style={{ padding: '7px 11px', border: '1px solid #ddd', borderRadius: 8, fontSize: 13 }}>
            <option value="">Todos los kinesiológos</option>
            {kines.map(k => <option key={k.id} value={k.id}>{k.apellido} {k.nombre}</option>)}
          </select>
        )}
